...
def demonstrate(run, emit):
    ...
    tools = create_tool_registry(run, emit)
    context = tools.get("calculate_change").context
    emit("拆解说法", "分别检查价格、早午盘比较、跨交易日涨跌幅")
    context.add_evidence("教学新闻（虚构）", DEMO_CLAIM, kind="demo")
    ...
    first = tools.execute("calculate_change", {"current": "620", "previous": "625"})
    emit("调用计算", "比较当前午盘 620.00 与上一交易日午盘 610.00")
    second = tools.execute("calculate_change", {"current": "620", "previous": "610"})
