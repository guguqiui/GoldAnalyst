...
function setMode(next) {
  if (busy) return;
  if (mode === 'live') liveInput = $('task').value;
  mode = next;
...
function render(run) {
  ...
  $('run-mode').textContent = run.mode === 'demo' ? 'TEACHING DEMO / 虚构样例' : 'LIVE INVESTIGATION / 联网调查';
...
$('investigation').onsubmit = async (event) => {
  ...
  finally { busy = false; $('start').disabled = false; $('start').innerHTML = (mode === 'demo' ? '运行教学演示' : '开始联网调查') + ' <span>→</span>'; }
};
async function init() {
  setMode('demo');
